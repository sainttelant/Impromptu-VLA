...
class PromptNuScenesSceneDescription(PromptNuScenesBlueprint):
    def __init__(self, nuscenes, **kwargs):
        super().__init__(nuscenes=nuscenes, cache_filename="nuscenes_scene_desc.json", container_out_key="scene_description", need_helper=True, **kwargs)
...
Q0. Descibe the image of current time (I1), but more focus on static scene information, i.e., background information instead of dynamic road agents.
...
Q2.3. What is the type of road in the scene? What are the challenges associated with it for driving? Are there traffic signs or signals? If yes, what do they indicate? 
...
Please think step by step to solve this problem. Subtitle your solutions by stages, i.e., A0, A1, etc. Note that A3 shall include ```json and ```.\n"""
    ...
    def format_output(self, helper_ret, container_out, container_in):
        ...
        scene_desc_string += f"{helper_ret['S_analysis']}\n"
        scene_desc_string += "</Scene Description>"
