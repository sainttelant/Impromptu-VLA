...
from data_engine.datasets.navsim.loaders.navsim.common.dataclasses import Frame, Annotations, Trajectory, Lidar
from data_engine.datasets.navsim.loaders.navsim.common.enums import BoundingBoxIndex, LidarIndex
from data_engine.datasets.navsim.loaders.navsim.planning.scenario_builder.navsim_scenario_utils import tracked_object_types
from data_engine.datasets.navsim.loaders.navsim.visualization.lidar import filter_lidar_pc, get_lidar_pc_color
from data_engine.datasets.navsim.loaders.navsim.visualization.config import BEV_PLOT_CONFIG, MAP_LAYER_CONFIG, AGENT_CONFIG, LIDAR_CONFIG
...
def add_annotations_to_bev_ax(ax: plt.Axes, annotations: Annotations, add_ego: bool = True, human_se2:StateSE2 = None, agent_se2:StateSE2 = None) -> plt.Axes:
    """
    ...
    for name_value, box_value in zip(annotations.names, annotations.boxes):
        ...
        def se2agent(se2: StateSE2, human_se2: StateSE2, agent_se2: StateSE2) -> StateSE2:
            """
            se2: StateSE2, se2 related to the human
            human_se2: StateSE2, human's pose
            agent_se2: StateSE2, agent's pose
            return: StateSE2, se2 related to the agent
            """
            # Convert SE2 states to transformation matrices
            human_to_world = human_se2.as_matrix()  # Human's pose relative to world
            agent_to_world = agent_se2.as_matrix()  # Agent's pose relative to world
            world_to_agent = np.linalg.inv(agent_to_world)  # Invert to get world-to-agent transform

            # Convert se2 (relative to human) to world coordinates
            se2_human_matrix = se2.as_matrix()  # Convert se2 to its transformation matrix
            se2_world_matrix = human_to_world @ se2_human_matrix  # Transform to world coordinates

            # Convert world coordinates to agent coordinates
            se2_agent_matrix = world_to_agent @ se2_world_matrix  # Transform to agent coordinates

            # Convert the resulting transformation matrix back to an SE2 state
            return StateSE2.from_matrix(se2_agent_matrix)

        
        if human_se2 is not None and agent_se2 is not None:
            agent_se2 = se2agent(StateSE2(x, y, heading), human_se2, agent_se2)
            x, y, heading = agent_se2.x, agent_se2.y, agent_se2.heading
