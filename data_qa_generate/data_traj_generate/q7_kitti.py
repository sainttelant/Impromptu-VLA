...
# Loop through both "testing" and "training" scenarios
for sp, (pt, ft) in zip(["testing", "training"], zip(pasts, futures)):
    results = []
    pts = []
    fts = []
    with open(pt, "r") as f:
    ...
    for k in Pts.keys():
        images = [
            f"{root}/data_tracking_image_2/{sp}/image_02/{str(k).zfill(4)}/{str(i).zfill(6)}.png" for i in frames[k]]

        for i in range(3, len(Pts[k]) - 10):
            assert len(Pts[k][i]) >= 3
            assert len(Fts[k][i]) >= 10
            results.append({"images": images[i], "messages": [{"role": "user", "content": user_p + avs(
                Pts[k][i], Fts[k][i])}, {"role": "assistant", "content": assistant_p + fps(Fts[k][i])}]})

    print(f"Number of results for {sp}: {len(results)}")
    
    # Determine output file name
    output_sp = "test" if sp == "testing" else "train"
    output_path = f"{project_root}/data_qa_results/kitti/q7_{output_sp}.json"
