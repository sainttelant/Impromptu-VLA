...
# print(project_root)
root = project_root / "data_raw" / "idd_multimodal" / "primary"
...
for pt, ft in zip(pasts, futures):
    ...
    for p in pts:
        if p['scene_id'] in frames:
            frames[p['scene_id']].append(p['image_idx'])
        else:
            frames[p['scene_id']] = [p['image_idx']]
    ...
    for k in Pts.keys():
        ...
        images = [
            f"{root}/{k.split('_')[0]}/leftCamImgs/{str(i).zfill(7)}.jpg" for i in frames[k]]
    ...
    print(len(results))
    output_path = f"{project_root}/data_qa_results/idd/q7.json"
